...
const SidebarContent = ({ sidebar_content, setSidebar_content }) => {
        ...
        children: [
          {
            ...
            value: dataProvince?.kecelakaan_tunggal?.value || 0,
            satuan: dataProvince?.kecelakaan_tunggal?.satuan || "Kasus",
            kenaikan: dataProvince?.kecelakaan_tunggal?.kenaikan,
          },
          {
            icon: <FaMotorcycle size={30} className="text-secondary" />,
            text: "Kecelakaan Motor",
            value: dataProvince?.kecelakaan_motor?.value || 0,
            satuan: dataProvince?.kecelakaan_motor?.satuan || "Kasus",
            kenaikan: dataProvince?.kecelakaan_motor?.kenaikan,
          },
          {
            icon: <BsCarFront size={30} className="text-secondary" />,
            text: "Kecelakaan Mobil",
            value: dataProvince?.kecelakaan_mobil?.value || 0,
            satuan: dataProvince?.kecelakaan_mobil?.satuan || "Kasus",
            kenaikan: dataProvince?.kecelakaan_mobil?.kenaikan,
          },
        ],
      },
    ].filter(Boolean);
...
>
  <div className="flex flex-col h-full w-1/4 p-2">
      ...
      <div className="flex justify-between items-start">
        <div>
          <p className="text-xl font-bold uppercase tracking-wide text-white">
            {dataProvince?.provinsi || "INDONESIA"}
          </p>
          {dataProvince?.provinsi !== "NASIONAL" && (
            <p className="text-sm text-gray-400 mt-1">
              Ibu Kota: {dataProvince?.ibu_kota || "-"}
            </p>
          )}
        </div>
        <button
    ...
    <div className="flex justify-center items-center py-6 bg-zinc-900/60 border-b border-gray-700">
      <img
        src={`/assets/logoprov/${
          (dataProvince?.id === "-" ? 1 : dataProvince?.id) || 1
        }.png`}
        className={dataProvince?.id === "-" ? "w-32" : "w-24"}
        alt="logoProv"
      />
    </div>
      ...
      {sidebarPortal.map((section, i) => (
        <div key={i} className="text-white">
          ...
          <div className="flex items-center justify-between px-4 py-3 bg-black/40 border border-gray-700 shadow-sm">
            ...
            <span className="text-sm text-yellow-400 font-medium">{section.year}</span>
          </div>
             ...
             onClick={() =>setSelectData(item)}
              ...
              >
                ...
                <div className="text-xl text-blue-400 pt-1">
                  {typeof item.icon === "object" ? item.icon : <i className={item.icon} />}
                </div>
                      ...
                      {item.children_2.map((sub, subIdx) => (
                        ...
                        >
                          <span
                            onClick={sub.onClick || (() => {})}
                            className={`${
                              sub.onClick && "hover:underline cursor-pointer"
                            }`}
...
{/* CONTENT STATISTIK */}
    {selectData &&          <StatistikContent data={selectData} setSelectData={setSelectData}/>
